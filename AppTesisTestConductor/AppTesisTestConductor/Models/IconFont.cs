using System;
using System.Collections.Generic;
using System.Text;
using Xamarin.Forms;

namespace AppTesisTestConductor.Models
{
	static class IconFont
	{
		public const string Star = "\ue800";
		public const string StarEmpty = "\ue801";
		public const string Heart = "\ue802";
		public const string Mail = "\ue803";
		public const string User = "\ue804";
		public const string Users = "\ue805";
		public const string OkCircled2 = "\ue806";
		public const string CancelCircled2 = "\ue807";
		public const string LockOpen = "\ue808";
		public const string Download = "\ue809";
		public const string Upload = "\ue80a";
		public const string Location = "\ue80b";
		public const string Delete = "\ue80c";
		public const string Save = "\ue80d";
		public const string Search = "\ue80e";
		public const string Eye = "\ue80f";
		public const string Truck = "\ue810";
		public const string CreditCard = "\ue811";
		public const string CreditCard1 = "\ue812";
		public const string Vcard = "\ue813";
		public const string Gmail = "\ue814";
		public const string Emo1 = "\ue815";
		public const string Emo2 = "\ue816";
		public const string Emo3 = "\ue817";
		public const string Emo4= "\ue818";
		public const string Emo5 = "\ue819";
		public const string Filter = "\ue81a";
		public const string Music = "\ue81b";
		public const string Camera = "\ue81c";
		public const string Logout = "\ue81e";
		public const string InfoCircled = "\ue81f";
		public const string Flag = "\ue820";
		public const string Phone = "\ue821";
		public const string Brujula = "\ue822";
		public const string Load = "\ue832";
		public const string Marca = "\uf097";
		public const string Siguiente = "\uf0a8";
		public const string Atras = "\uf0a9";
		public const string Chat = "\uf0e5";
		public const string FlagCheckered = "\uf11e";
		public const string DirectionArrow = "\uf124";
		public const string LockOpenAlt = "\uf13e";
		public const string Dollar = "\uf155";
		public const string Like = "\uf164";
		public const string DisLike = "\uf165";
		public const string Google = "\uf1a0";
		public const string Cab = "\uf1b9";
		public const string Taxi = "\uf1ba";
		public const string Share = "\uf1e0";
		public const string CcVisa = "\uf1f0";
		public const string Bicycle = "\uf206";
		public const string Bus = "\uf207";
		public const string UserSecret = "\uf21b";
		public const string Motorcycle = "\uf21c";
		public const string Facebook = "\uf230";
		public const string Whatsapp = "\uf232";
		public const string CalendarCheckO = "\uf274";
		public const string Map = "\uf279";

		public static Color ColorBordes = Color.ForestGreen;
		public static Color ColorFondo = Color.White;
		public static Color ColorIcono = Color.DarkOliveGreen;
		public static Color ColorTexto = Color.Black;
		public static Color ColorGreen = Color.GreenYellow;
		public static Color ColorRed = Color.OrangeRed;

		public const string Font = "fontello";
		public const double FontSize = 100;

	}
}
